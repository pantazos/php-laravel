    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('extras') ? 'is-invalid' : '' }}" name="extras[]" id="extras" multiple>
                    @foreach($extras as $id => $extra)
                        <option value="{{ $id }}" {{ in_array($id, old('extras', [])) ? 'selected' : '' }}>{{ $extra }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('promo_code') ? 'is-invalid' : '' }}" name="promo_code_id" id="promo_code_id">
                    @foreach($promo_codes as $id => $entry)
                        <option value="{{ $id }}" {{ old('promo_code_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('parts_cost') ? 'is-invalid' : '' }}" type="number" name="parts_cost" id="parts_cost" value="{{ old('parts_cost', '0') }}" step="0.01">
